...
完成 CampusFix 起始版本的可运行验证，并修复创建工单时「位置」与「描述」字段颠倒的缺陷，使主分支达到可交付状态。

### 任务清单

1. 修复创建工单时 `room` / `description` 参数顺序颠倒的问题（关联缺陷 Issue）
2. 为创建工单补充回归测试，覆盖字段正确落库
3. 补充创建工单表单的输入校验（标题、位置非空）

### 验收标准

- 新建工单后，详情页「位置」显示用户填写的位置，「描述」显示用户填写的描述，不再颠倒
- `pytest -v` 全部通过，`test_create_ticket` 不再失败
- 分支命名符合 `type/编号-描述` 约定，提交保持原子性
- 代码经 Review 通过后合并进 main
